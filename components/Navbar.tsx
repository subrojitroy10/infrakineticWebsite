      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-full z-50 mt-3 min-w-[15rem] rounded-md border border-white/12 bg-ink-900 p-2"
          >
            {items.map((child) => (
              <Link
                key={child.href}
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled ? 'border-b border-white/10 bg-ink-900/95' : 'bg-ink-900/70'
      }`}
    >
      <nav className="container-page flex h-16 items-center justify-between md:h-20">
        ...
        <div className="hidden items-center gap-4 md:flex">
          ...
          <Link
            href="/early-access"
            className="text-xs font-semibold text-white/58 transition-colors hover:text-gold-300"
          >
            Early Access
          </Link>
          <Link href="/briefing" className="btn-primary !rounded-md !px-4 !py-2.5 text-xs">
            Request a briefing
          </Link>
        </div>
          ...
          >
            <div className="container-page flex max-h-[70vh] flex-col gap-1 overflow-y-auto py-4">
              ...
              <Link
                href="/early-access"
                onClick={() => setOpen(false)}
                className="border-t border-white/10 px-3 py-4 text-base font-semibold text-gold-300 hover:text-gold-200"
              >
                Join Early Access
              </Link>
              <Link
                href="/briefing"
                onClick={() => setOpen(false)}
                className="mt-2 rounded-md border border-action bg-action px-3 py-3 text-center text-base font-semibold text-action-text"
              >
                Request a briefing
              </Link>
            </div>
