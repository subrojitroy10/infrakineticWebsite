  ...
  return (
    ...
    >
      ...
      <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {migrationEngine.steps.map((step, i) => (
          <Reveal key={step.n} variant="up" delay={i * 0.06}>
            <div className="h-full border-t border-white/10 py-6 pr-5">
      ...
      <Reveal variant="fade" className="mt-12">
        <div className="rounded-md border border-white/10 bg-ink-800/30 p-6 md:p-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {migrationEngine.connectors.map((connector) => (
              <span
                key={connector}
                className="border-t border-violet-400/25 pt-2 text-sm font-medium text-violet-300"
              >
                {connector}
              </span>
            ))}
        ...
        {migrationEngine.differentiators.map((point, index) => {
          ...
          return (
            <Reveal key={point.title} variant="up" delay={index * 0.05}>
              <div className="h-full border-t border-white/10 py-6 pr-5 transition-colors hover:border-gold-400/30">
      ...
      <Reveal variant="fade" className="mt-12">
        <div className="flex flex-col items-start gap-4 border-y border-white/10 py-6 sm:flex-row sm:items-center sm:justify-between md:py-8">
