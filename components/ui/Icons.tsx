...
const make = (paths: React.ReactNode) => {
  const Icon: React.FC<IconProps> = ({ size = 18, className = '', ...rest }) => (
    <svg {...baseProps} width={size} height={size} className={className} {...rest}>
      {paths}
    </svg>
  )
...
export const Activity = make(<path d="M22 12h-4l-3 9L9 3l-3 9H2" />)
