...
import { KpiTileRow, KpiTile, Sparkline, TrendDelta, StatusBadge, EntityAvatar, SectionHeader, DataTable } from '../shared/components'
import {
  Target,
  Users,
  Wallet,
...
const radarColumns = [
  { key: 'name', header: 'Account', sortable: true, width: '200px', render: (val, row) => (
    <div className="flex items-center gap-3">
      <EntityAvatar name={val} type="org" size="sm" />
      <div>
        <p className="font-medium text-white">{val}</p>
        <p className="text-[10px] text-white/40">{row.segment || 'Enterprise'}</p>
      </div>
    </div>
  )},
  { key: 'lifecycle_state', header: 'State', sortable: true, render: (val) => <StatusBadge value={val} family="lifecycle" size="sm" dot /> },
  { key: 'health_score', header: 'Health', sortable: true, render: (val, row) => (
    <div className="flex items-center gap-2">
      <span className="font-semibold text-white">{val}</span>
      <TrendDelta value={row.health_delta} />
    </div>
  )},
  { key: 'arr', header: 'ARR', sortable: true, render: (val) => <span className="font-semibold text-white">{val}</span> },
  { key: 'next_renewal', header: 'Renewal', sortable: true, render: (val) => <span className="text-white/70">{val}</span> },
  { key: 'overdue_count', header: 'Overdue', sortable: true, render: (val) => <StatusBadge value={val > 0 ? 'overdue' : 'paid'} family="invoice" size="sm" /> },
  { key: 'open_cases', header: 'Cases', sortable: true, render: (val) => <span className="text-white/70">{val}</span> },
  { key: 'action_required', header: 'Action', sortable: true, render: (val) => <span className="text-gold-300 font-medium text-sm">{val}</span> },
]
...
export default function CX360() {
  return (
    <div className="relative">
        ...
        <div className="container-page">
          ...
          >
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4 text-sm text-gold-300">
              <StatusBadge value="add-on" family="lifecycle" size="sm" dot variant="positive" />
              <span className="text-white/50">Requires Commerce</span>
              <span className="text-white/50">|</span>
              <StatusBadge value="atomic" family="lifecycle" size="sm" dot variant="positive" />
      ...
      {/* Account Shell — Health & Interventions */}
      <Section
        id="account-shell"
      ...
      >
        <Reveal variant="fade" className="mt-14">
          <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
            ...
            >
              <ParallaxCard depth={18} className="h-full p-6 md:p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <StatusBadge value="heuristic" family="lifecycle" size="sm" variant="warning" />
                </div>
            ...
            {/* Right: Interventions + Action Rail */}
            <motion.div
              initial={{ opacity: 0, x: 24 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              <ParallaxCard depth={20} className="p-6 md:p-8 border-gold-300/30 bg-gold-300/[0.04]">
                <div className="inline-flex items-center gap-2 mb-4">
                  <StatusBadge value="learned" family="lifecycle" size="md" dot variant="positive" />
                </div>
...
// Need to add AlertTriangle and HeartPulse icons