          ...
          <ParallaxCard depth={18} className="p-6 md:p-8 overflow-x-auto">
            <div className="min-w-[800px]">
              <div className="flex items-center justify-center gap-2 flex-wrap">
                {[
                  { id: 'prospect', label: 'Prospect', color: 'prospect' },
                  { id: 'qualified', label: 'Qualified', color: 'qualified' },
                  { id: 'in_pipeline', label: 'In Pipeline', color: 'in_pipeline' },
                  { id: 'committed', label: 'Committed', color: 'committed' },
                  { id: 'onboarding', label: 'Onboarding', color: 'onboarding' },
                  { id: 'active', label: 'Active', color: 'active' },
                  { id: 'at_risk', label: 'At Risk', color: 'at_risk' },
                  { id: 'renewing', label: 'Renewing', color: 'renewing' },
                  { id: 'expanding', label: 'Expanding', color: 'expanding' },
                  { id: 'churned', label: 'Churned', color: 'churned', sticky: true },
                  { id: 'won_back', label: 'Won Back', color: 'won_back', sticky: true },
                ].map((state) => (
                  <motion.div
                    key={state.id}
                    layout
      ...
      >
        <Reveal variant="fade" className="mt-14">
          <div className="grid gap-6 md:grid-cols-3">
            {[
              { icon: Lock, title: 'Enforced at the database, not just in code', desc: 'Every table carries a tenant-isolation policy, forced even for administrative connections — there\'s no privileged path that skips the check.' },
