...
/**
 * DataTable — Sticky header, left accent bar per row, hover, row-click,
 * sortable headers, empty state, footer (Showing X to Y of Z + pagination + per-page selector)
 */
export default function DataTable({
  ...
  const isSelected = (row) => selectedKeys.includes(row[keyField])
  ...
  if (!data.length) {
    return (
      <div className={`rounded-2xl border border-white/10 bg-white/[0.02] p-12 text-center ${className}`}>
        ...
        {emptyState?.action && (
          <button className="btn-primary">{emptyState.action.label}</button>
        )}
      </div>
    )
  }
              ...
              {selectable && (
                <th className="w-12 px-4 py-3 text-left">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        onSelectionChange?.(paginatedData.map((r) => r[keyField]))
                      } else {
                        onSelectionChange?.([])
                      }
                    }}
